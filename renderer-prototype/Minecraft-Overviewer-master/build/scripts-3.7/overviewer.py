...
#################
import os
#sys.path.append('C:/Users/yangm/Desktop/cool stuff/Minecraft-Overviewer-master_2/Minecraft-Overviewer-master')
sys.path.append(os.getcwd())
outdir = sys.argv[-1].replace('\\','/')
if os.path.exists(outdir) == False:
    os.mkdir(outdir)
outdir = sys.argv[-1].replace('\\','/') + '/Chunkdata'
f = open('outdir.txt','w')
f.write(outdir)
f.close()
...
f = open(f'{outdir}/outdir.txt','w')
f.write(outdir)
f.close()
f = open(f'{outdir}/domain.txt','w')
f.write('')
f.close()

from overviewer_core import sw_analysis
#################
...
def main():
    ...
    ##########
    try:
        worldsummary = sw_analysis.read_chunkdata(outdir)
        # f = open(f'{outdir[:-10]}/worldsummary.txt','w')
        # for s in worldsummary:
        #     f.write(f'{s}\n')
        # f.close()
        # logging.info("Saved world summary.")
        try:
            rarity = sw_analysis.get_rarity(worldsummary)
            
            # f = open(f'{outdir[:-10]}/worldrarity.txt','w')
            # for i in range(len(rarity[0])):
            #     f.write(f'{rarity[0][i]}  {rarity[1][i]}\n')
            # f.close()
            # logging.info('Saved world rarity.')
            try:
                import json
                txtfeatures = sw_analysis.get_features(worldsummary,rarity)
                (area, biome, resource, feature) = txtfeatures
                out = {
                    'attributes': []
                }
                for a in area + resource:
                    data = a[0]
                    out['attributes'].append({
                        'trait_type': data[0],
                        'value': data[1],
                    })
                for a in biome + feature:
                    data = a[0]
                    out['attributes'].append({
                        'value': data,
                    })

                with open(f'{outdir[:-10]}/worldfeatures.json','w',encoding='utf-8') as f:
                    json.dump(out, f, indent=2)
                
                ff = open(f'{outdir[:-10]}/worldfeatures.txt','w',encoding='utf-8')
                for feat in txtfeatures:
                    ff.write(str(feat)+'\n')
                ff.close()
                logging.info('Saved world features text.')
            except Exception as e:
                logging.info('Cannot save features text.' + str(e))
        except:
            logging.info('Cannot save rarity features.')
    except:
        logging.info('Cannot save world summary.')

    try:
        sw_analysis.tilestopng(outdir[:-10])
        logging.info("Generated big png file.")
    except:
        logging.info("Cannot generate big png file.")
        pass
    ##########
    ##########
    ##########
