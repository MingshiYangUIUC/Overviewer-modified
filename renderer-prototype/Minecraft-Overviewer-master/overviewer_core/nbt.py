...
class MCRFileReader(object):
    ...
    def load_chunk(self, x, z):
        ...
        try:
            NNN = NBTFileReader(data, is_gzip=is_gzip).read_all()
            #print(type(NNN),dir(NNN))
            outdir = open('outdir.txt','r').read()
            '''f = open(f'{outdir}/c_{X}_{Z}_.txt','w')
            f.write(str(NNN))
            f.close()'''
            if X in range(-1000000,1000000) and Z in range(-1000000,1000000):
                f = open(f'{outdir}/domain.txt','a')
                f.write(f'{X}    {Z}\n')
                f.close()
            try:
                sw_analysis.inspect_save_chunk(NNN,X,Z,outdir)
            except:
                pass
            return NNN
